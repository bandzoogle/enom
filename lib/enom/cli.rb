...
module Enom
  class CLI
    def initialize
      ...
      return if Enom::Client.username && Enom::Client.password
      ...
      if File.exist?(file)
        ...
        raise InvalidCredentials,
              'Please provide a username/password as arguments create a config file with credentials in ~/.enomconfig'
      end
    end

    def execute(command_name, args, options = {})
      command = commands[command_name]
      raise CommandNotFound, "Unknown command: #{command_name}" unless command
