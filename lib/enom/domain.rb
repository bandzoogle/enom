...
module Enom
  class Domain
    ...
    def initialize(attributes)
      ...
      #
      @name = if attributes['sld'] && attributes['tld']
                [attributes['sld'], attributes['tld']].join('.')
              elsif attributes['domainname']
      ...
      set_extended_domain_attributes(attributes) if attributes['services'] && attributes['status']
    end

    # Find the domain (must be in your account) on Enom
    def self.find(name)
      sld, tld = parse_sld_and_tld(name)
      @response = Client.request('Command' => 'GetDomainInfo', 'SLD' => sld,
                                 'TLD' => tld)['interface_response']['GetDomainInfo']
      Domain.new(@response)
    end
    ...
    def self.check_details(name)
      ...
      @response = Client.request(
        ...
        'SLD' => sld,
        'TLD' => tld
      )['interface_response']['Domains']['Domain']

      @response.each do |key, _value|
    ...
    def self.all(options = {})
      limit = options.delete('limit') || Float::INFINITY
      domains = []
      finished = false
      index = 1
    ...
    def self.delete!(name, options = {})
      ...
      @response = Client.request({ 'Command' => 'DeleteRegistration', 'SLD' => sld,
                                   'TLD' => tld }.merge(options))['interface_response']
    ...
    def locked
      unless defined?(@locked)
        @response = Client.request('Command' => 'GetRegLock', 'SLD' => sld,
                                   'TLD' => tld)['interface_response']['reg_lock']
        @locked = @response == '1'
      end
      @locked
    end
    ...
    def get_extended_domain_attributes
      sld, tld = name.split('.')
      attributes = Client.request('Command' => 'GetDomainInfo', 'SLD' => sld,
                                  'TLD' => tld)['interface_response']['GetDomainInfo']
